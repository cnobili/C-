...
namespace JMH_SSIS_Gen_File2Table
{
...
class FileLoader
{
  ...
  // Class Variables
  private static String auditColsOption;
  
  public static void Usage()
  {
    Console.WriteLine();
    Console.WriteLine("------------------------------------------------------------------------------------------------");
    Console.WriteLine("Usage: SSIS_Pkg_File2Table arg1 arg3 arg3 ... argN");
    Console.WriteLine();
    ...
    Console.WriteLine();
    Console.WriteLine(">SSIS_Pkg_File2Table LoadFile flatFile delimiter schema tableName truncate|append connectString auditCols|noAuditCols");
    Console.WriteLine(">SSIS_Pkg_File2Table LoadDir  directory filePattern delimiter schema truncate|append connectString auditCols|noAuditCols");
    Console.WriteLine(">SSIS_Pkg_File2Table LoadFromFileList listOfFiles auditCols|noAuditCols");
    Console.WriteLine("------------------------------------------------------------------------------------------------");
    Console.WriteLine();
    Console.WriteLine("Example> SSIS_Pkg_File2Table LoadFile c:\\mydir\\myfile.dat , dbo tableName Truncate \"Integrated Security=SSPI;Initial Catalog=theDatabase;Data Source=theServer;\" auditCols");
    Console.WriteLine();
  ...
  public static void UpdateAuditCols(String schema, String tableName, String identityColumn, long maxID, String filename, DateTime loadDate, String connectString)
  {
  
    var connection = new SqlConnection(connectString);
    String sqlUpdate = String.Format("update {0}.{1} set LoadDate = '{2}', FileName = '{3}' where {4} > {5}", schema, tableName, loadDate, filename, identityColumn, maxID);
  ...
  public static long GetMaxID(String schema, String tableName, String identityColumn, String connectString)
  {
  
    var connection = new SqlConnection(connectString);
    String sqlSelCount = "select coalesce(max(" + identityColumn + "), 0) from " + schema + "." + tableName;
    var command = new SqlCommand(connectString);
  ...
  public static long GetTableRecCount(String schema, String tableName, String identityColumn, long maxID, String connectString)
  {
  
    var connection = new SqlConnection(connectString);
    String sqlSelCount = String.Format("select count(*) from {0}.{1} where {2} > {3}", schema, tableName, identityColumn, maxID);
    var command = new SqlCommand(connectString);
  ...
  public static void CreateTable(String schema, String tableName, String identityColumn, String connectString, string[] columns)
  {
    ...
    sql.Append("  " + identityColumn + " BIGINT IDENTITY(1, 1),");
    ...
    if (auditColsOption.ToUpper() == "AUDITCOLS")
    {
      sql.Append(" LoadDate DateTime,");
      sql.Append( "FileName NVARCHAR(4000)");
      sql.Append(")");
    }
    else
    {
  ...
  public static void DropTableColumn(String schema, String tableName, String column, String connectString)
  {
  ...
  public static void LoadFile(String filename, String delim, String schema, String tableName, String loadType, String connectString)
  {
    ...
    if (!TableExists(schema, tableName, connectString))
    {
      ...
      CreateTable(schema, tableName, identityColumn, connectString, columns);
    }
    ...
    long maxID = GetMaxID(schema, tableName, identityColumn, connectString);
    ...
    if (auditColsOption.ToUpper() == "AUDITCOLS")
      UpdateAuditCols(schema, tableName, identityColumn, maxID, filename, DateTime.Now, connectString);
    
    Console.WriteLine("Records in file = {0}, header record excluded", GetFileRecCount(filename) - 1);
    Console.WriteLine("Records loaded into table = {0}", GetTableRecCount(schema, tableName, identityColumn, maxID, connectString));
    
    if (auditColsOption.ToUpper() != "AUDITCOLS")
      DropTableColumn(schema, tableName, identityColumn, connectString);
